...
public class FilterDispatcher implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            String newURI = uri.replace("/CarRental", "");
            if (newURI.length() > 1) {
                url = newURI;
            }
            int authorizationFlag = 0;//ko co role
            if (url.contains("/user")) {
                authorizationFlag = 1;//can phai co role admin
            } else if (url.contains("/admin")) {
                authorizationFlag = 2;//can phai co role user
            }
            switch (authorizationFlag) {
                case 1:
                        ...
                        if (user != null) {
                            switch (user.getRole()) {
                                case "ROLE_USER":
                                    ...
                                    int totalRecords = searchDAO.getAllRecordsOfThatSearch("", "", null, null);
                                    ...
                                    break;
                                case "ROLE_ADMIN":
                                    req.getRequestDispatcher("/admin/home.jsp").forward(request, response);
                                    break;
                                default:
            ...
            t.printStackTrace();
            req.setAttribute("MESSAGE", "Loi khong xac dinh");
            req.getRequestDispatcher(url).forward(request, response);
        }
    }
