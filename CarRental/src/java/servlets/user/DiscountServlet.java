...
public class DiscountServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DiscountServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (discountCode != null && !discountCode.isEmpty()) {
                ...
                DiscountCoupon discountCoupon = discountDAO.getCouponByID(discountCode);
                if (discountCoupon != null) {
                    HttpSession session = request.getSession();
                    Date now = Date.valueOf(LocalDate.now());
                    int total = (Integer) session.getAttribute("TOTAL");
                    if (now.after(discountCoupon.getExpiredDate())) {
                        message = "Ma giam da het han";
                    } else {
                        total = total - (total * discountCoupon.getDiscount() / 100);
                        message = "Ban duoc giam " + discountCoupon.getDiscount() + "% trong tong hoa don";
                        session.setAttribute("DISCOUNT_FLAG", true);
                        session.setAttribute("TOTAL", total);
                    }
                } else {
                    message = "Khong ton tai ma giam nay";
                }
            } else {
                message = "Ma giam khong duoc trong";
            }
