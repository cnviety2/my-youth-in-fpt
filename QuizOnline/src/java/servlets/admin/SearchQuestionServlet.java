...
public class SearchQuestionServlet extends HttpServlet {
    ...
    private final Logger logger = Logger.getLogger(SearchQuestionServlet.class);
    ...
    public void init() throws ServletException {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(SearchQuestionServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (searchContent == null || searchStatus == null || searchSubject == null) {
                throw new NullPointerException();
            } else {
                AdminDAO dao = new AdminDAO();
                Boolean status = null;
                Integer subjectID = null;
                if (!searchStatus.equals("none")) {
                    switch (searchStatus) {
                        case "true":
                            status = true;
                            break;
                ...
                if (!searchSubject.equals("none")) {
                    subjectID = Integer.parseInt(searchSubject);
                }
                int page = 0;
                if (pageString != null && !pageString.isEmpty()) {
                    page = Integer.parseInt(pageString);
                }
                List<SearchQuestionDTO> listQuestion = dao.searchPart1(searchContent, status, subjectID, page);
                int totalRecords = dao.getAllRecordsOfSearchResult(searchContent, status, subjectID);
                ...
                if (listQuestion != null) {
                    ...
                    List<Answer> listAnswer = dao.searchPart2(listQuestionID);
                    for (SearchQuestionDTO dto : listQuestion) {
                    ...
                    request.setAttribute("SEARCH_SUBJECT", searchSubject);
                    request.setAttribute("TOTAL_PAGES", totalPages);
                    request.setAttribute("STANDING_PAGE", page);
                } else {
            ...
            logger.error("Error at SearchQuestionServlet:" + e.toString());
        } catch (Exception e) {
